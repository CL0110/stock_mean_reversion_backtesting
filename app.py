...
if run:
    with st.spinner("Downloading data and running backtest..."):
        ...
        if rebalance_freq == "Weekly":
            rebal_dates = stocks.resample("W").last().index
        elif rebalance_freq == "Monthly":
            rebal_dates = stocks.resample("ME").last().index
        else:
            rebal_dates = stocks.index
        # Strategy returns
        ...
        for i in range(lookback + 1, len(stocks)):
            dt = stocks.index[i]
            if dt in rebal_dates or rebalance_freq == "Daily":
                z = zscore.iloc[i - 1]
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=strat_dd.index, y=strat_dd,  name="Mean Reversion", fill="tozeroy", line=dict(color="#0366d6")))
        fig2.add_trace(go.Scatter(x=bench_dd.index, y=bench_dd,  name="DIA ETF",        fill="tozeroy", line=dict(color="#f0883e")))
        ...
        st.download_button("⬇️ Download Results CSV", results_df.to_csv().encode(),
